...
# run_layer1_final.sh
#
# Runs the full Layer 1 (EM strip) cell-energy reweighting pipeline for 4 variants:
#   eta_loose    — 14 eta bins, loose isolation
#   eta_tight    — 14 eta bins, tight isolation
#   eta_pt_loose — 14 eta × 6 pT bins, loose isolation
#   eta_pt_tight — 14 eta × 6 pT bins, tight isolation
#
# Phases: fill (parallel) → hadd inclusive → plot → chi2 → compendium.
#
# Usage:
#   cd scripts/data_mc
...
#
#   VARIANTS="eta_loose" bash run_layer1_final.sh   # subset
#
set -euo pipefail
...
echo "============================================"
echo "  Layer 1: Final cell-energy reweighting"
echo "  Started: $(date)"
echo "============================================"

SCRIPTDIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTBASE="${SCRIPTDIR}/../../output/Layer_1"
...
# ── Phase 1: Fill (parallel) ─────────────────────────────────────
echo ">>> Phase 1: Filling histograms (parallel)"
FILL_PIDS=()
for var in "${VARIANTS_RUN[@]}"; do
    ...
    for ch in "${CHANNELS[@]}"; do
        for sc in "${SCENARIOS_FILL[@]}"; do
            ...
            mkdir -p "${BASE}/${ch}/${sc}"
            root -l -b -q "fill_histograms_layer1.C(\"${ch}\", \"${sc}\", \"${BASE}\", \"${BIN}\", \"${ISO}\")" \
...
# ── Phase 2: hadd inclusive ──────────────────────────────────────
echo ""
echo ">>> Phase 2: hadd inclusive"
for var in "${VARIANTS_RUN[@]}"; do
    BASE="${OUTBASE}/${var}"
    for ch in "${CHANNELS[@]}"; do
...
for var in "${VARIANTS_RUN[@]}"; do
    ...
    for ch in "${CHANNELS[@]}"; do
        for sc in "${SCENARIOS_ALL[@]}"; do
            ...
            root -l -b -q "plot_shower_shapes_layer1.C(\"${ch}\", \"${sc}\", \"${BASE}\", \"${BIN}\", \"${ISO}\")" || \
                echo "  WARN: plot_shower_shapes_layer1 failed for ${var}/${ch}/${sc}"
            root -l -b -q "plot_cell_profiles_layer1.C(\"${ch}\", \"${sc}\", \"${BASE}\", \"${BIN}\", \"${ISO}\")" || \
                echo "  WARN: plot_cell_profiles_layer1 failed for ${var}/${ch}/${sc}"
        done
    done
done
...
# ── Phase 4: Chi2 tables ─────────────────────────────────────────
echo ""
echo ">>> Phase 4: Chi-squared tables"
for var in "${VARIANTS_RUN[@]}"; do
    BASE="${OUTBASE}/${var}"
    REPORT="${BASE}/report"
    mkdir -p "${REPORT}"
    echo "  chi2: ${var}"
    root -l -b -q "extract_chi2_layer1.C(\"${BASE}\", \"${REPORT}\")" || \
        echo "  WARN: extract_chi2_layer1 failed for ${var}"
done

# ── Phase 5: Compendia ───────────────────────────────────────────
echo ""
echo ">>> Phase 5: Compendia"
COMP_SCRIPT="${OUTBASE}/make_compendiums.py"
if [[ -f "${COMP_SCRIPT}" ]]; then
    python3 "${COMP_SCRIPT}" || echo "  WARN: make_compendiums (Layer 1) failed"
else
    echo "  WARN: ${COMP_SCRIPT} not found; skipping compendia"
fi
