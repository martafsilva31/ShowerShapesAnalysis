/// \file plot_rphi.C
/// \brief Overlay fudged / unfudged / computed R_phi with ratio panel.
///
/// Usage:
...
void plot_rphi(const char* infile  = "../output/rphi_Zeeg.root",
               const char* outbase = "../output/plots/rphi_Zeeg") {

    TFile* f = TFile::Open(infile);
    if (!f || f->IsZombie()) { std::cerr << "Cannot open " << infile << "\n"; return; }

    TH1F* h_unfudged = (TH1F*)f->Get("h_rphi_unfudged");
    TH1F* h_fudged   = (TH1F*)f->Get("h_rphi_fudged");
    TH1F* h_computed = (TH1F*)f->Get("h_rphi_computed");
    ...
    r_fudged->GetXaxis()->SetTitle("R_{#phi}");
