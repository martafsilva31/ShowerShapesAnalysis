#!/usr/bin/env python3
"""
Compute R_eta from Layer 2 cell energies and compare with stored values.

R_eta = E237 / E277 = E(3 eta x 7 phi) / E(7 eta x 7 phi) in Layer 2
...
# Compile C++ helper once — runs at native speed inside the event loop
ROOT.gInterpreter.Declare(r"""
#include <vector>
double compute_reta_cpp(const std::vector<double>& cells) {
    ...
    return (E_7x7 > 0) ? E_3x7 / E_7x7 : -999.0;
}
""")


def main():
    parser = make_parser('Compute R_eta from cell energies')
    ...
    # Enable only needed branches for speed
    tree.SetBranchStatus("*", 0)
    tree.SetBranchStatus("photon.7x11ClusterLr2E", 1)
    tree.SetBranchStatus("photon.7x11ClusterLr2Eta", 1)
    tree.SetBranchStatus("photon.reta", 1)
    tree.SetBranchStatus("photon.unfudged_reta", 1)
    ...
    h_fudged = ROOT.TH1F("h_reta_fudged", ";R_{#eta};Events", 100, 0.5, 1.05)
    h_unfudged = ROOT.TH1F("h_reta_unfudged", ";R_{#eta};Events", 100, 0.5, 1.05)
    h_computed = ROOT.TH1F("h_reta_computed", ";R_{#eta};Events", 100, 0.5, 1.05)
    h_diff_comp_unfudged = ROOT.TH1F("h_reta_diff_computed_unfudged",
                                      ";#Delta R_{#eta};Events", 100, -0.02, 0.02)
    h_diff_fudged_unfudged = ROOT.TH1F("h_reta_diff_fudged_unfudged",
                                        ";#Delta R_{#eta};Events", 100, -0.02, 0.02)
    h_2d = ROOT.TH2F("h_reta_2d", ";Unfudged R_{#eta};Computed R_{#eta}",
    ...
    leaf_fudged = tree.GetLeaf("photon.reta")
    leaf_unfudged = tree.GetLeaf("photon.unfudged_reta")
    ...
    for i in range(max_ev):
        ...
        reta_comp = ROOT.compute_reta_cpp(cell_vec)
        if reta_comp < 0:
            continue
        ...
        h_fudged.Fill(rf)
        h_unfudged.Fill(ru)
        h_computed.Fill(reta_comp)
        h_diff_comp_unfudged.Fill(reta_comp - ru)
        h_diff_fudged_unfudged.Fill(rf - ru)
        h_2d.Fill(ru, reta_comp)

        arr_fudged[n_good] = rf
        arr_unfudged[n_good] = ru
        arr_computed[n_good] = reta_comp
        n_good += 1
