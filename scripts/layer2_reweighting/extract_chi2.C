...
//   chi2_tables_eta_mu.tex — Per-(eta,mu) chi2 table (only when binning="eta_mu")
//   chi2_tables_eta_pt.tex — Per-(eta,pT) chi2 table (only when binning="eta_pt")
//
// Usage:
//   cd scripts/data_mc
//   root -l -b -q 'extract_chi2.C'                              # eta-only variant
//   root -l -b -q 'extract_chi2.C("...","...","eta_mu")'       # eta x mu variant
///////////////////////////////////////////////////////////////////////////////
    ...
    const char* binning   = "eta")
{
    ...
    auto secBinLabel = [&](int p) -> std::string {
        if (usePtBins) return ptBinLabel(p);
        if (useMuBins) return muBinLabel(p);
        return "";
    };
    ...
    // ============================================================
    // (Optional) per-(eta,sec) chi2 table — only for eta_pt / eta_mu
    // ============================================================
    if (!useSecBins) return;

    TString secFile = Form("%s/chi2_tables_eta_%s.tex",
                           reportDir, secTag.Data());
    std::ofstream secOut(secFile.Data());
    secOut << "% Auto-generated by extract_chi2.C ("
           << binning << ") — do not edit by hand\n";
    secOut << "% Per-(|eta|," << secTag << ") chi2 tables.\n\n";

    // Only produce for llgamma / inclusive (most stats)
    const int IC = 2; // llgamma
    for (int is = 0; is < nScen; ++is) {
        TString path = Form("%s/%s/%s/histograms.root",
                            baseDir, channels[IC], scenarios[is]);
        TFile* fSec = TFile::Open(path, "READ");
        if (!fSec || fSec->IsZombie()) {
            if (fSec) delete fSec;
            continue;
        }
        std::cout << "Reading per-(eta," << secTag << "): " << path << std::endl;

        for (int iv = 0; iv < nVar; ++iv) {
            TString label = Form("tab:chi2-eta%s-%s-%s",
                                 secTag.Data(), scenarios[is], vars[iv]);

            secOut << "\\begin{table}[htbp]\n";
            secOut << "\\centering\n";
            secOut << "\\caption{Per-$(|\\eta|,\\langle\\mu\\rangle)$ $\\chisq$"
                      " for " << varTeX[iv] << ", "
                   << chanTeX[IC] << ", " << scenTeX[is] << " scenario.}";
            secOut << "\n\\label{" << label << "}\n";
            secOut << "\\resizebox{\\textwidth}{!}{%\n";

            // Columns: |eta| range | MC | M1 | M2 | for each sec bin
            secOut << "\\begin{tabular}{r";
            for (int p = 0; p < nSecBins; ++p)
                secOut << "|rrr";
            secOut << "}\n";
            secOut << "\\toprule\n";

            // Header row 1: sec-bin labels
            secOut << "$|\\eta|$ range";
            for (int p = 0; p < nSecBins; ++p)
                secOut << " & \\multicolumn{3}{" << (p < nSecBins-1 ? "c|" : "c")
                       << "}{$" << secBinLabel(p) << "$}";
            secOut << " \\\\\n";
            ...
            for (int ie = 0; ie < kNEtaBins; ++ie) {
                ...
                for (int p = 0; p < nSecBins; ++p) {
                    TString suf = Form("_eta%02d_%s%02d", ie, secTag.Data(), p);
                    TH1D* hd  = (TH1D*)fSec->Get(Form("h_%s_data%s",    vars[iv], suf.Data()));
                    TH1D* hmc = (TH1D*)fSec->Get(Form("h_%s_mc%s",      vars[iv], suf.Data()));
                    TH1D* hm1 = (TH1D*)fSec->Get(Form("h_%s_mc_M1%s",   vars[iv], suf.Data()));
                    TH1D* hm2 = (TH1D*)fSec->Get(Form("h_%s_mc_M2%s",   vars[iv], suf.Data()));

                    double c0 = -1, c1 = -1, c2 = -1;
                    if (hd && hd->GetEntries() >= 50) {
                        TH1D* nd  = normClone(hd,  "sc_d");
                        TH1D* nmc = normClone(hmc, "sc_mc");
                        TH1D* nm1 = normClone(hm1, "sc_m1");
                        TH1D* nm2 = normClone(hm2, "sc_m2");
                        c0 = (nd && nmc) ? chi2ndf(nmc, nd) : -1;
                        c1 = (nd && nm1) ? chi2ndf(nm1, nd) : -1;
                        c2 = (nd && nm2) ? chi2ndf(nm2, nd) : -1;
                        delete nd; delete nmc; delete nm1; delete nm2;
                    }
                    secOut << " & " << fmtChi2(c0)
                           << " & " << fmtChi2(c1)
    ...
    secOut.close();
    std::cout << "Written: " << secFile << std::endl;
}
