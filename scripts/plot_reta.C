/// \file plot_reta.C
/// \brief Overlay fudged / unfudged / computed R_eta with ratio panel.
///
/// Usage:
...
void plot_reta(const char* infile  = "../output/reta_Zeeg.root",
               const char* outbase = "../output/plots/reta_Zeeg") {

    TFile* f = TFile::Open(infile);
    if (!f || f->IsZombie()) { std::cerr << "Cannot open " << infile << "\n"; return; }

    TH1F* h_unfudged = (TH1F*)f->Get("h_reta_unfudged");
    TH1F* h_fudged   = (TH1F*)f->Get("h_reta_fudged");
    TH1F* h_computed = (TH1F*)f->Get("h_reta_computed");
    ...
    // ATLAS Internal label + sample info
    TLatex lat;
    lat.SetNDC();
    ...
    r_fudged->GetXaxis()->SetTitle("R_{#eta}");
