...
correction methods — a flat additive shift and an energy-dependent profile
correction — are derived and validated. Using the full sample of 9.6~million
events, the energy-dependent method achieves $\chi^2/n_\text{df}$ reductions
of 90--95\% relative to uncorrected Monte Carlo across all pseudorapidity bins
...
Three shower shape variables are examined:
\begin{itemize}
  ...
  \item $w_{\eta\,2} = \sqrt{\langle\eta^2\rangle_E
        - \langle\eta\rangle_E^2}$, the energy-weighted shower width in
        $\eta$, computed in a $3\eta\times 5\phi$ window.
\end{itemize}
...
The energy-dependent correction (Method~2, described below) uses profile
...
the energy-dependent correction.
...
data--MC discrepancies in photon shower shapes: data tends to exhibit narrower
showers than simulation, corresponding to a redistribution of energy from
peripheral cells toward the cluster centre.
...
\begin{itemize}
  ...
  \item $b_k$ is a \textbf{fixed bias pattern} determined by the Chebyshev
        ...
        \end{equation}
        This pattern enhances the central core and suppresses the tails,
        producing a narrowing of the shower.
...
\paragraph{Method~1: Flat additive shift.}
...
\paragraph{Method~2: Energy-dependent profile correction.}
...
\textbf{Method~1} applies a flat shift:
\begin{equation}
  f_k^\text{corr} = f_k^\text{MC} + \Delta_k^{(n)}.
  \label{eq:m1apply}
\end{equation}

\textbf{Method~2} applies an energy-dependent correction obtained by
interpolating the profile histogram at the value of the MC fraction:
\begin{equation}
...
After the correction, the three shower shape variables are recomputed from
...
The shower width $w_{\eta\,2}$ is computed in a $3\eta\times 5\phi$ window:
\begin{equation}
  w_{\eta\,2} = \sqrt{
  ...
  \label{eq:weta2}
\end{equation}
where $\eta_i = (i-1)\times\Delta\eta$ for cell row indices
$i\in\{0,1,2\}$ in the 3-row sub-window, and $\Delta\eta = 0.025$ is the
cell size in the second sampling layer. Since $w_{\eta\,2}$ is defined as a
second central moment, it is invariant under translation, and the use of
relative positions yields results equivalent to using absolute cell $\eta$
coordinates.
...
and can reverse the sign of the flat-shift corrections. Negative cell energies
...
\subsection{Shower width calculation}
\label{sec:adaptations:weta2}

The second-layer shower width $w_{\eta\,2}$ is defined as the
energy-weighted RMS of the cell $\eta$ positions in a $3\times5$ window:
\begin{equation}
  w_{\eta\,2} = \sqrt{
    \frac{\sum_{k\in3\times5} E_k\,\eta_k^2}{\sum_{k\in3\times5} E_k}
    - \left(
        \frac{\sum_{k\in3\times5} E_k\,|\eta_k|}{\sum_{k\in3\times5} E_k}
      \right)^{\!2}
  }.
  \label{eq:weta2_ref}
\end{equation}
The original method evaluates this using the absolute cell $\eta$
coordinates stored in a dedicated per-cell branch. Since this branch is not
available in the MC23e ntuples, the calculation uses relative integer
positions $\eta_i \in \{0,\,\Delta\eta,\,2\Delta\eta\}$ (corresponding to
the three $\eta$ rows in the $3\times5$ window, with
$\Delta\eta = 0.025$) in place of absolute $\eta$ coordinates.

This substitution is mathematically valid because $w_{\eta\,2}$ is a
second central moment (variance), which is invariant under translation of
the $\eta$ coordinate:
\begin{equation}
  \text{Var}(\eta) \;=\; \langle\eta^2\rangle - \langle|\eta|\rangle^2
  \;=\; \langle(\eta{+}c)^2\rangle - \langle|\eta{+}c|\rangle^2
  \;=\; \text{Var}(\eta{+}c)
\end{equation}
for any constant offset~$c$, provided that all $\eta_k$ values within the
window have the same sign (guaranteed when using $|\eta_k|$ as in the
original formula). The resulting $w_{\eta\,2}$ values are expressed in
units of $\Delta\eta$ rather than absolute pseudorapidity, differing by a
constant scale factor. For the closure test --- which compares MC,
pseudo-data, and corrected MC distributions all computed with the
\emph{same} formula --- this difference cancels exactly.
...
$w_{\eta\,2}$ $\eta$ coords & Absolute cell $\eta$ values    & Relative positions ($\Delta\eta=0.025$) \\
...
\Cref{fig:5pct_eta0} shows the closure plots for all three shower shape
...
is consistent across all three variables.
...
\begin{figure}[htbp]
  ...
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.32\textwidth}
    \includegraphics[page=1,width=\textwidth]{../output/closure_test_5pct/closure_weta2.pdf}
    \caption{$w_{\eta\,2}$}
    \label{fig:5pct_eta0_weta2}
  \end{subfigure}
  \caption{%
    Closure plots for $R_\eta$~(a), $R_\phi$~(b), and $w_{\eta\,2}$~(c)
...
\Cref{fig:5pct_eta13} shows the same three variables at 5\% distortion in
...
\begin{figure}[htbp]
  ...
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.32\textwidth}
    \includegraphics[page=13,width=\textwidth]{../output/closure_test_5pct/closure_weta2.pdf}
    \caption{$w_{\eta\,2}$}
  \end{subfigure}
  \caption{%
...
$\chi^2/n_\text{df}$ values for all three shower shape variables, in all
...
\paragraph{Both methods achieve substantial closure.}
In all $|\eta|$ bins, at all distortion levels, both Method~1 and Method~2
reduce the $\chi^2/n_\text{df}$ relative to the uncorrected MC. The
corrections consistently move the reweighted distributions closer to the
pseudo-data target.

\paragraph{Method~2 consistently outperforms Method~1.}
The energy-dependent profile correction achieves $\chi^2/n_\text{df}$ values
typically 5--10 times smaller than the flat shift correction. For example, at
$\delta=0.05$ in $|\eta|$ bin~0, the $R_\eta$ values are: MC~=~0.0059,
M1~=~0.0046, M2~=~0.0005. The improvement is most dramatic for $R_\eta$,
which is the variable most sensitive to the lateral energy distribution.
...
Nevertheless, both methods still achieve significant $\chi^2$ reduction.

\paragraph{Consistency across shower shape variables.}
All three variables ($R_\eta$, $R_\phi$, $w_{\eta\,2}$) show similar patterns
...
\begin{itemize}
  \item \textbf{Method~1} reduces the $R_\eta$ $\chi^2/n_\text{df}$ by
    approximately 50--80\% relative to uncorrected MC.
  \item \textbf{Method~2} reduces the $R_\eta$ $\chi^2/n_\text{df}$ by
    approximately 90--95\% relative to uncorrected MC.
  \item Similar reduction factors are observed for $R_\phi$ and
    $w_{\eta\,2}$.
\end{itemize}
...
The test demonstrates that both correction methods — a flat additive shift per
cell and an energy-dependent profile correction — successfully recover the
target pseudo-data distributions across all 13~populated $|\eta|$ bins and
three distortion levels. The energy-dependent correction (Method~2) achieves
consistently superior performance, reducing the $\chi^2/n_\text{df}$ by
approximately 90--95\% relative to uncorrected MC, compared to 50--80\% for
the flat shift (Method~1).
...
5\%), and for all three shower shape variables tested ($R_\eta$, $R_\phi$,
$w_{\eta\,2}$).
...
energy-dependent parameterisation provides a significant improvement over the
flat-shift baseline. The closure test provides the necessary validation to
...
bin, the three shower shape variables ($R_\eta$, $R_\phi$, $w_{\eta\,2}$)
...
% --- Helper command for one eta-bin row (3 subfigs) -------------------------
