...
\begin{frame}{Motivation}
  ...
  \textbf{Photon identification in ATLAS} relies on rectangular cuts
  on shower shape variables, computed from cell energies in the
  EM calorimeter.
  \vspace{8pt}

  \textbf{Problem:} well-known mismodelling of calorimeter shower
  shapes in the Geant4 simulation.
  \vspace{4pt}

  Several sources of mismodelling:
  \begin{itemize}
    \item Detector geometry description in simulation
    \item Cross-talk — signal leakage between LAr cells
          due to non-ideal connections
    \item Magnetic field modelling
  \end{itemize}
  \vspace{6pt}

  \begin{itemize}
    \item Need to correct these effects to better represent data
    \item Improving G4 is very time-consuming
    \item Currently, direct data-driven corrections to the shower
          shapes are derived, the so-called \textbf{fudge factors} (FF)
  \end{itemize}
...
for \textbf{Run~3 conditions}, building on the Run~2 method
...
\begin{enumerate}
  \item \textbf{Validate} the cell-energy reweighting method with Run~3 MC
        via closure tests
  \item \textbf{Apply} corrections to real Run~3 data
  \item \textbf{Extend} the approach to \textbf{Layers~1 and~3}
        (strip and back layers)
  \item \textbf{Explore normalising flows} as an alternative correction
        method — learn the full multivariate data--MC transformation
        at cell level, capturing inter-cell correlations and
        conditioning on $p_\text{T}$, $\eta$, conversion type
\end{enumerate}
\end{frame}
