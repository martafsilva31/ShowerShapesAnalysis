...
# Submit data24 DAOD_EGAM3 NTupleMaker jobs to the grid
# Reads dataset list from NTupleMaker/script/data24_13p6TeV.DAOD_EGAM3.25ns.p61xx.txt
# Analysis: eegamma (Z->eegamma), year 2024, DAOD cell-links (-d 0)
#
# Usage:
#   bash grid/submit_data24_egam3.sh          # submit all 157 runs
#   bash grid/submit_data24_egam3.sh --test   # submit first run only (1 file)

WORKDIR="/project/atlas/users/mfernand/QT/ShowerShapes/NTupleMaker_workspace"
SCRIPTDIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DSLIST="${WORKDIR}/source/NTupleMaker/script/data24_13p6TeV.DAOD_EGAM3.25ns.p61xx.txt"
